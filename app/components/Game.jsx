...
export default class Game extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      gameOver: false,
      winner: null,
      playerBoards: [battleship.boardWithShips(this.props.ships[1]),
  ...
  render() {
    ...
    <div>
      ...
      <GameBoard board={this.state.playerBoards[0]}
                 canFire={!this.state.hasFired}
                 onFire={this.fire.bind(this)}
                 visible={whoseTurn === 0}/>
      <GameBoard board={this.state.playerBoards[1]}
                 canFire={!this.state.hasFired}
                 onFire={this.fire.bind(this)}
                 visible={whoseTurn === 1}/>
      {gameOver ? <h1>Player {this.state.winner} wins!</h1> : ''}
      {(hasFired && !gameOver) ? <button onClick={this.switchTurn}>Next Player</button> : ''}
    </div>
    )
  }
};
